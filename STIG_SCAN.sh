...
# 
#
# Synopsis - SHELL Script to run DISA HPUX STIGS 
...
if [ "$(uname -r)" = "B.11.31" ]; then
 OS="HPUX"
 chmod +x $STIGDIR/$OS/*
 BLANKCKL="$STIGDIR/CKL_Templates/blank_hpux.ckl"
# $STIGDIR/$OS/prestage.sh
 ipaddress="$(getip $servername)"
 macaddress="$(arp $servername | cut -f 4 -d " ")"
else
 exit
fi
